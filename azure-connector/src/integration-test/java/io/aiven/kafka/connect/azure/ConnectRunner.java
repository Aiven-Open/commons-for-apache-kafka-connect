...
final class ConnectRunner {
    ...
    void start() {
        ...
        final AllConnectorClientConfigOverridePolicy allowOverride = new AllConnectorClientConfigOverridePolicy(); // NOPMD
        final MemoryOffsetBackingStore offsetStore = new MemoryOffsetBackingStore() {
            @Override
            public Set<Map<String, Object>> connectorPartitions(final String connectorName) {
                throw new UnsupportedOperationException();
            }
        };
        final Worker worker = new Worker(workerId, time, plugins, config, offsetStore, allowOverride);
        herder = new StandaloneHerder(worker, kafkaClusterId, allowOverride);
