...
import io.aiven.kafka.connect.azure.testutils.AzureBlobAccessor;
...
class AbstractIntegrationTest<K, V> {
    ...
    static void setUpAll() throws IOException, InterruptedException {
        ...
        azurePrefix = "azure-connector-for-apache-kafka-test-"
                + ZonedDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME) + "/";

        final File testDir = Files.createTempDirectory("azure-connector-for-apache-kafka-test-").toFile();

        pluginDir = new File(testDir, "plugins/azure-connector-for-apache-kafka/");
